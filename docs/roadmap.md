...
> **Note:** This roadmap predates the [north star architecture](architecture.md). The phases below need to be revisited to align with the updated metamodel (workspaces, agent vs runtime separation, task attempts/plans/steps, etc.). The general progression — local loop first, then production concerns — remains correct.
...
- Call OpenAI API directly (hardcoded provider, no router) for reasoning
...
**Deliverable:** A local Rust process that registers with SpacetimeDB, picks up a task, reasons about it using an LLM, executes shell commands, posts observations, and completes the task.
...
**Goal:** Replace the hardcoded LLM calls with a proper routing service.
...
- K8s operator: warm pod pools, workspace PVC management, agent lifecycle
- Credential management: K8s Secrets mounted per task, revoked after
...
- Tool registry: `tools` table, capability discovery
...
- Approval queue: `approvals` table, `aule_suggest` tool, approval UI in frontend
- Events: `events` table, pub/sub via subscriptions, `cleanup_events` TTL
- Messages: user-agent communication per task
